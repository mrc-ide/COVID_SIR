...
plot_combined_parameter_correlation_heatmap <- function(stoch_sir_fit, det_sir_fit, det_adap_sir_fit) {
  ...
  ggplot(df, aes(x = beta, y = gamma, fill = ..density..)) +
    ...
    scale_fill_viridis_c() +  # Colorblind-friendly palette
    facet_wrap(~ Type) +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5, size = 14),
...
plot_filter_samples <- function(samples_df) {
  ggplot(samples_df, aes(x = Beta, y = LogPosterior, group = Filter, color = Filter)) +
    geom_line() +
    geom_point() +
    scale_color_manual(values = c("Deterministic" = "blue", "Stochastic" = "red")) +
    labs(
      x = expression(beta),
      y = "Log-posterior",
